...
python3 $fiji_path$corpus_construction$iaa_calculations/gold_standard_summary_stats.py -gs_path=$fiji_path$ignorance_base_all_data_corpus_path$output_results$knowtator_projects$all_data_results -article_path=$articles -annotation_path=$concept_annotation -all_lcs_path=$all_lcs_path -section_info_path=$section_info_path -section_format=$section_format



##preprocess the docs to get the pmcid sentence files so we have the ignorance statements vs not
python3 $fiji_path$preprocess_corpus/preprocess_docs.py -craft_path=$fiji_path$ignorance_base_all_data_corpus_path$output_results$knowtator_projects$all_data_results -articles=$articles -concept_annotation=$concept_annotation -ontologies=$ontologies -output_path=$fiji_path$ignorance_base_all_data_corpus_path$output_results$knowtator_projects$all_data_results$tokenized_files -pmcid_sentence_path=$pmcid_sentence_files_path -corpus=$corpus --all_lcs_path=$fiji_path$ignorance_base_all_data_corpus_path$output_results$knowtator_projects$all_data_results$all_lcs_path


python3 $fiji_path$new_articles_path/new_article_stats.py -corpus_path=$fiji_path$ignorance_base_all_data_corpus_path$output_results$knowtator_projects$all_data_results -article_path=$articles -annotation_path=$concept_annotation -all_lcs_path=$all_lcs_path -pmcid_sentence_file_path=$pmcid_sentence_files_path