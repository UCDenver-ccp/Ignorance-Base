...
#ontologies='incomplete_evidence'
...
python3 $fiji_path$eval_path/final_output_knowtator_format.py -ontologies=$ontologies -all_lcs_path=$fiji_path$corpus_path$all_lcs_path -ontology_file_path=$fiji_path$corpus_path$ontology_file_path -broad_categories=$broad_categories -article_path=$fiji_path$ignorance_base_corpus$articles -xml_folder=$output_folder -bionlp_folder=$bionlp_folder -algos=$algos -result_folders=$results_folders -results_path=$scratch_path$ignorance_base_corpus$output_results -file_types=$file_types -evaluation_files=$all_files
...
do
    ...
    do
        echo $j
        mkdir $scratch_path$ignorance_base_corpus$output_results$knowtator_projects$i$underscore$j
        cd $scratch_path$ignorance_base_corpus$output_results$knowtator_projects$i$underscore$j
...
#        for e in "${eval[@]}"
#        do
            cp $scratch_path$ignorance_base_corpus$articles/*$article_ext $articles
            cp $scratch_path$ignorance_base_corpus$output_results$i/$output_folder$j/*$annotation_ext$algo$annotation_ext2 $concept_annotation
